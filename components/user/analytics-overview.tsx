...
import { TrendingUp, TrendingDown, DollarSign, Receipt, BarChart3, Target } from "lucide-react"
  ...
  const overviewCards = [
    {
      ...
      change: `${isPositiveChange ? "+" : ""}${stats.monthlyChange.toFixed(1)}%`,
      ...
      {overviewCards.map((card) => (
        <Card key={card.title} className="bg-white/80 backdrop-blur-sm border-white/20">
          ...
          <CardContent>
            ...
            {card.change && (
              <div className={`flex items-center text-sm ${card.changeColor}`}>
